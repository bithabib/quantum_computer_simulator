...
an extrapolation setting---training only on small circuits and testing on the two
largest, unseen circuit sizes ($n=11,12$)---the predictor retains
$R^2 = \RsqExtrap{}$, indicating that the architectural signature of a barren
plateau transfers to unseen circuit sizes. Permutation importance
...
We emphasize honest framing: we make \emph{no} claim of quantum advantage. The
...
Killoran~\cite{schuld2022killoran} argue the field should de-emphasize ``beating
classical.'' We adopt this stance: the object we predict is a property of quantum
circuits, but the predictor and its value proposition are entirely classical. We
are not aware of prior work that casts barren-plateau severity as a directly
\emph{predicted} regression/classification target from cheap architectural
features and tests extrapolation across qubit count.
...
exactly with the in-house \texttt{qsim} NumPy simulator~\cite{eshyana2026}, so
labels reflect ideal (noiseless) trainability.
...
implementation~\cite{pedregosa2011scikit}, inspired by
LightGBM~\cite{ke2017lightgbm}). Features are standardized for the scale-sensitive
models. We
...
picture. First, the linear baseline is far behind on regression ($R^2=0.27$ vs.\
...
(regression) structure. Second, the
...
$R^2$ slightly exceeds the hold-out value, which we do not read as an easier task
in general: the $n=11,12$ test set is dominated by saturated, strongly barren
global-cost circuits whose labels are comparatively easy to predict. The result
should therefore be read as evidence that small-circuit labels transfer to the
largest sizes we can still simulate, not as unbounded extrapolation. Because the
ground-truth cost of labeling grows exponentially with qubit count, even this
limited downward-in-cost transfer is what an ansatz-screening tool needs.
...
label noise at the trainable end. (v)~The label is the variance of the gradient
with respect to a single fixed middle-layer rotation; in principle the variance can
depend on which parameter is probed. (vi)~The barren/trainable cutoff
